...
(function() {
  ...
  class ImageEditor {
    ...
    _onMouseDown(e) {
      ...
      if (this.currentTool === TOOL_TYPES.TEXT) { this._startTextInput(coords.x, coords.y); this.isDrawing = false; return; }
    ...
    _startTextInput(x, y) {
      ...
      this.textInput.style.cssText = `position:absolute;left:${rect.left - wrapper.getBoundingClientRect().left + x * scaleX}px;top:${rect.top - wrapper.getBoundingClientRect().top + y * scaleY}px;color:${this.currentColor};font-size:${this.fontSize * scaleY}px;`;
      this.textInputX = x; this.textInputY = y;
      wrapper.appendChild(this.textInput); this.textInput.focus();
      this.textInput.addEventListener('blur', () => setTimeout(() => this._finishTextInput(), 100));
    }
